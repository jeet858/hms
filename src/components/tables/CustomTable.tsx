          ...
          <div className="w-full rounded-lg border border-gray-300 bg-gray-100">
            <div className="flex rounded-t-lg w-full bg-gray-200">
              {props.headers.map((header, index) => (
                <div
                  key={index}
                  className={`flex text-[14px]/[16.41px] p-[0.5%] border-r border-gray-300 ${index==0 ? "w-[5%]" :index==1?"w-[15%]":index==2?"w-[10%]":index==3?"w-[15%]":index==5?"w-[12%]":index==6?"w-[14%]":"w-[10%]"} font-bold`}
                >
                  {header}
                </div>
              ))}
            ...
            {paginatedData.map((item, rowIndex) => (
              <div key={rowIndex} className="flex border-t w-full border-gray-300">
                {props.headers.map((header, colIndex) => (
                  <div
                    key={colIndex}
                    className={`flex p-[0.5%] text-[14px]/[16.41px] border-r border-gray-300 ${colIndex==0 ? "w-[5%]" :colIndex==1?"w-[15%]":colIndex==2?"w-[10%]":colIndex==3?"w-[15%]":colIndex==5?"w-[12%]":colIndex==6?"w-[14%]":"w-[10%]"}`}
                  >
                    {item[header]}
                  </div>
                ))}
